...
task main() {
	...
	*/
	...
	//pull row forward to scoring zone
	mmToDrive = 1610.0;
	armPowerLevel = 100;
	drivePowerLevel = 100;
	targetClawPosition 	= PICKUP_BLOCK_CLAW;
	...
	//Drive forward until hitting the wall 900 mm
	mmToDrive = 750.0;
	armPowerLevel = 100;
	drivePowerLevel = 100;
	targetClawPosition 	= FULL_OPEN_CLAW;
	...
	//Drive forward  while drifting right for 800mm??

	//Grab row

	//Pull Row backwards while drifting hard left for 1600 mm
...
void moveRobot( int targetClawPosition, int targetWristPosition, int targetArmPosition, int armPowerLevel, int drivePowerLevel, float mmToDrive, int drift) {
	...
	int 		degreesOfTravel, targetWheelPosition;
	...
	while(motorsMoving) {
		...
		if ( !driveFinished) {
			...
			if ( drift == DRIFT_RIGHT ) {
				setMotorTarget(leftMotor,		 targetWheelPosition,   drivePowerLevel);
				setMotorTarget(rightMotor,	 targetWheelPosition,   drivePowerLevel * .98);
			}
			else if ( drift == DRIFT_MODERATE_RIGHT) {
				setMotorTarget(leftMotor,		 targetWheelPosition,   drivePowerLevel);
				setMotorTarget(rightMotor,	 targetWheelPosition,   drivePowerLevel * .94);
			}
			else if ( drift == DRIFT_FAR_RIGHT) {
				setMotorTarget(leftMotor,		 targetWheelPosition,   drivePowerLevel);
				setMotorTarget(rightMotor,	 targetWheelPosition,   drivePowerLevel * .90);
			}
			else if ( drift == DRIFT_LEFT ) {
				setMotorTarget(leftMotor,		 targetWheelPosition,   drivePowerLevel *.98);
				setMotorTarget(rightMotor,	 targetWheelPosition,   drivePowerLevel);
			}
			else {
				if ( (abs(leftMotorValue) - 3) > abs(rightMotorValue) ) {
					setMotorTarget(leftMotor,		 targetWheelPosition,   drivePowerLevel * .94);
					setMotorTarget(rightMotor,	 targetWheelPosition,   drivePowerLevel);
				}
